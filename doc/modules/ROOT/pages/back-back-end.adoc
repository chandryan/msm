...
- `msm::back` is using {cpp}03 and is kept for backwards compatibility
- `msm::back11` is using {cpp}11 and should be preferred over `msm::back` where possible, as it removes some pre-{cpp}11 limitations (mpl::vector of limited size)
- `msm::backmp11` is using {cpp}17 and should be used by default if possible. It provides additional features and significantly better compile times and runtime performance. It is further described in a xref:./backmp11-back-end.adoc[separate page]
...
xref:front-tutorial/basic-front-end.adoc#event-hierarchy[event hierarchy].
...
with xref:#run-to-completion[run-to-completion] semantics. You can also enqueue the events and
...
events generated during xref:#run-to-completion[run-to-completion] or with `enqueue_event`, one
...
_Where should I define a state machine?_:

The tutorials are implemented in a simple cpp source file for simplicity.

_I want to model dynamic behavior of a class as a state machine, how should I define the state machine?_

Usually you'll want to implement the state machine as an attribute of the class. Unfortunately, a concrete state machine is a typedef, which cannot be forward-declared. This leaves you with two possibilities:
