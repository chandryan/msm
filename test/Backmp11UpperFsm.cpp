...
namespace
{
    ...
    {     
        ...
        p.process_event(EnterSubFsm()); 
        BOOST_CHECK_MESSAGE(p.machine_entries == 2, "SM entry not called correctly");

        p.process_event(EnterSubFsm()); 
        BOOST_CHECK_MESSAGE(p.machine_entries == 3, "SM entry not called correctly");

        p.process_event(ExitSubFsm()); 
        BOOST_CHECK_MESSAGE(p.machine_exits == 1, "SM exit not called correctly");

        p.process_event(ExitSubFsm()); 
        BOOST_CHECK_MESSAGE(p.machine_exits == 2, "SM exit not called correctly");

        p.stop(); 
        BOOST_CHECK_MESSAGE(p.machine_exits == 3, "SM exit not called correctly");
    }
}